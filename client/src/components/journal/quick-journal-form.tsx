...
// Simple schema for quick journal entries
const quickJournalSchema = z.object({
  content: z.string().min(1, "Journal content cannot be empty"),
  coverImage: z.string().optional(),
});

type QuickJournalFormValues = z.infer<typeof quickJournalSchema>;
...
export function QuickJournalForm() {
  ...
  const form = useForm<QuickJournalFormValues>({
    resolver: zodResolver(quickJournalSchema),
    defaultValues: {
      content: "",
      coverImage: "",
    },
  ...
  const createJournalMutation = useMutation({
    mutationFn: async (data: QuickJournalFormValues) => {
      // Create a simple journal with auto-generated title from first 5 words
  ...
  const onSubmit = (values: QuickJournalFormValues) => {
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex-grow space-y-4 w-full">
            ...
            {coverImage && (
              <div className="relative rounded-md overflow-hidden">
                <img src={coverImage} alt="Journal cover" className="w-full h-auto max-h-[200px] object-cover rounded-md" />
                <Button
                  type="button"
                  size="sm"
                  variant="destructive"
